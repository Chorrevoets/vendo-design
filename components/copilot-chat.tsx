...
export function CopilotChat({ chatId, initialPrompt, mode = "full", inputPlaceholder }: CopilotChatProps) {
  ...
  return (
    <div className={`flex flex-col ${mode === "embedded" ? "h-full" : "h-screen"} w-full ${mode === "embedded" ? "" : "chat-gradient-bg"}`}>
      {messages.length > 0 ? (
        <>
          <div
            className={
              mode === "embedded"
                ? "flex-1 overflow-y-auto p-3 max-[750px]:px-2"
                : "absolute top-16 bottom-20 left-0 right-0 overflow-y-auto p-3 max-[750px]:px-2"
            }
            style={{
              ...(mode !== "embedded" && typeof window !== "undefined" && (window.location.pathname === "/chat" || window.location.pathname.startsWith("/chats/"))
                ? {
          ...
          {mode === "embedded" ? (
            <div className="pt-0 pb-3 mt-auto bg-transparent">
              <div className="px-3 max-[750px]:px-2">
                  ...
                  <div className="relative shadow-lg rounded-full w-full">
                    <div className="relative">
                      <Input
                        ...
                        placeholder={inputPlaceholder ?? (isMobile ? "Ask about your data" : "Ask me about your key results, customers, or business insights...")}
                    ...
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                      ...
                      <div className="relative group">
                        <Button
                          ...
                          variant="ghost"
                          size="sm"
                          className={`h-8 w-8 p-0 hover:bg-gray-100 text-gray-500 rounded-full ${isSpeaking ? "bg-blue-200 text-blue-600" : "bg-gray-200"} ${messages.length === 0 ? "opacity-50" : ""}`}
                        ...
                        <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-50">
                          <div className="bg-gray-900 text-white text-xs px-2 py-1 rounded-md whitespace-nowrap">
                            {isSpeaking ? "Stop Speech" : messages.length > 0 ? "Play Last Response" : "No messages to play"}
                          </div>
                        </div>
                      </div>
                    </div>
